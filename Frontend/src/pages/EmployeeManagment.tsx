...
import {
  ...
  useGetEmployeesByOrganizationQuery,
  useUpdateUserMutation,
  // Add useInviteUserMutation if needed directly in this component,
  // but it's handled inside InviteUserForm now.
} from "../features/User/userApi";
import useCurrentUser from "../hooks/useCurrentUser";
import type { User } from "../types/User";
import {
...
const EmployeeManagement = () => {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<User | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false); // General submitting state for dialogs
  const [errorSnackbarOpen, setErrorSnackbarOpen] = useState(false);
  ...
  // This function is the callback for when InviteUserForm successfully invites a user
  const handleInviteNewEmployee = (inviteData: InviteUserInput) => {
    // The actual API call is handled inside InviteUserForm.
    // Here, we just handle the post-submission actions.
    refetch(); // Refresh the employee list to show the newly added user
    setShowInviteUserDialog(false); // Close the invitation dialog
    // No need to manage isSubmitting here, as InviteUserForm manages its own loading state for the submission button.
  };
  ...
  const handleConfirmDelete = async (employee: User) => {
    ...
    setIsSubmitting(true); // Indicate that a delete operation is in progress
    try {
      await deleteUser(employee._id).unwrap();
  ...
  const handleSaveEdit = async (updatedData: Partial<User>) => {
    ...
    setIsSubmitting(true); // Indicate that an edit operation is in progress
    try {
      await updateUser({ userId: selectedEmployee._id, data: updatedData }).unwrap();
